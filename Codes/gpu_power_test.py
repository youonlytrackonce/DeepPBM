...
vidNumber = 1
nSample = 599  # number of samples to be loaded for the training
loadPath = '../Data/MOT17-02-DPM/img1_resize/'
frst = io.imread(loadPath + '000001.jpg')
height, width, nCh = frst.shape
imgs =  np.empty([nSample, nCh, height, width])
print('loading the video frames ....')
for i in range(nSample):
    imName = loadPath + '%06d.jpg' % (i+1)
...
#Path parameters
save_PATH = './Result/BMC2012/MOT17-02-DPM_h_layer_5_{}_batch_size_{}_epoch_num_{}'.format(h_layer_5, batch_size, epoch_num)
if not os.path.exists(save_PATH):
    os.makedirs(save_PATH)

PATH_vae = save_PATH + '/betaVAE_BMC2012_MOT17-02-DPM-%2d' % latent_dim
# Restore
Restore = False
...
class VAE(nn.Module):
    def __init__(self):
        ...
        self.dconv1 = nn.ConvTranspose2d(h_layer_5, h_layer_4, kernel_size=kernel_size, stride=stride, padding = 0, output_padding = 0)
        self.dbn1 = nn.BatchNorm2d(h_layer_4, eps = 1e-5, momentum = 0.1, affine = True, track_running_stats = True)
        self.dconv2 = nn.ConvTranspose2d(h_layer_4, h_layer_3, kernel_size=kernel_size, stride=stride, padding = 0, output_padding = 0)
        self.dbn2 = nn.BatchNorm2d(h_layer_3, eps = 1e-5, momentum = 0.1, affine = True, track_running_stats = True)
        self.dconv3 = nn.ConvTranspose2d(h_layer_3, h_layer_2, kernel_size=kernel_size, stride=stride, padding = 0, output_padding = 0)
        self.dbn3 = nn.BatchNorm2d(h_layer_2, eps = 1e-5, momentum = 0.1, affine = True, track_running_stats = True)
        self.dconv4 = nn.ConvTranspose2d(h_layer_2, h_layer_1, kernel_size=kernel_size, stride=stride, padding = 0, output_padding = 0)
        self.dbn4 = nn.BatchNorm2d(h_layer_1, eps = 1e-5, momentum = 0.1, affine = True, track_running_stats = True)
        self.dconv5 = nn.ConvTranspose2d(h_layer_1, 3, kernel_size=kernel_size, padding = 0, stride=stride, output_padding = 1)
    ...
    def Decoder(self, z):
        ...
        dh3 = self.relu(self.dbn1(self.dconv1(dh2.view(-1, h_layer_5, 18, 33))))
